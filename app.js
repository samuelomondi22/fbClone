...
app.get("/", (req, res) => {
  res.send("hello world it is amazing");
});
//6tryegd847uyehj
...
//database
mongoose.connect(
  // 'mongodb+srv://samuel:Rawfootage@cluster0-pea1r.mongodb.net/test?retryWrites=true&w=majority',
  "mongodb://localhost:27017/facebook_mern",
...
app.use(bodyParser.json()); //add body to req => req.
...
app.post(
  ...
  [
    ...
    check("lastname")
      ...
      .isLength({ min: 2 })
      .withMessage("Lastname should be at least 2 letters")
      .matches(/^([A-z]|\s)+$/)
...
app.get("/api/current_user", function(req, res) {
  if (req.session.user) {
    User.findById(req.session.user._id).then(function(user) {
      res.send({
        //this object is going to response.data in axios
        _id: user._id,
...
app.post("/api/post", function(req, res) {
  Post.create({
    ...
    .catch(function(error) {
      res.send({
        status: "error",
        message: "Could not create post in database"
      });
    });
});
...
app.post(
  ...
  [
    ...
    check("lastname")
      .isLength({ min: 2 })
      .withMessage("Last name should be at least 4 letters")
  ],
  function(req, res) {
    var errors = validationResult(req);
